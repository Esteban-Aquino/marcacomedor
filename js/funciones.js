...
function ajax_error(vmens) {
    ...
    swalCerrar();
    swalError('', 'Error de al comunicarse con el servidor, favor volver a intentar'); 
}
...
function swalErrorComida(vtitulo, vtexto) {
    Swal.fire({
        allowOutsideClick: true,
        type: "error",
        title: vtitulo,
        text: vtexto,
        customClass: "swal-error-comida",
        timer: 3000
    }).then( ()=>
        $('#nrotarjeta').focus()
        );
...
function swalCorrectoComida(vtitulo, vmensaje) {
    Swal.fire({
        type: 'success',
        title: vtitulo,
        text: vmensaje,
        customClass: "swal-correcto-comida",
        timer: 3000
    }).then( ()=>
        $('#nrotarjeta').focus()
        );;
...
function cargarModal(pRuta, pFocoEntrar, pFocoSalir) {
